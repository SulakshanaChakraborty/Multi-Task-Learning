...
class UNet(nn.Module):
    def __init__(self, enc_chans=(3, 64, 128, 256, 512, 1024), dec_chans=(1024, 512, 256, 128, 64), num_class=2,
                 retain_dim=True, output_size=(256, 256)):
    ...
    def forward(self, x):
        ...
        out = unet.Decoder(enc_chans[::-1][0], enc_chans[::-1][1:])
        out = unet.Convolution1(out)
        if unet.retain_dim:
            out = F.interpolate(out, unet.out_sz)
        return out
